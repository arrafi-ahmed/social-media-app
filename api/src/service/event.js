...
exports.findWallEvents = (userId, startDate, endDate, category, page) => {
  const itemsPerPage = 10;
  const offset = (page - 1) * itemsPerPage;
  let values = [];
  let sql =
    "select e.*, c.full_name, c.image from event_post e join cuser c on e.user_id = c.id where e.user_id = $1";
  ...
  if (category) {
    sql += " and e.category = $4";
    values.push(category);
  }
  sql += " ORDER BY e.date DESC, e.id DESC limit $5 offset $6";
...
exports.findBrowseEvents = (userId, startDate, endDate, category, page) => {
  const itemsPerPage = 10;
  const offset = (page - 1) * itemsPerPage;
  let values = [];
  ...
  if (startDate && endDate) {
    sql += " and e.date between $2 and $3";
    values.push(startDate);
    values.push(endDate);
  }
  if (category) {
