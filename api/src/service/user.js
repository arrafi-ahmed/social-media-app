...
exports.getProfileWSettings = (userId) => {
  const sql =
    "select c.id, c.full_name, c.email, c.date_of_birth, c.country, c.image, c.created_at," +
    " us.email_new_event_notification, us.email_update_event_notification, us.email_new_comment_notification from" +
    " cuser c join user_settings us on c.id = us.user_id where c.id = ?";
  return db.getRow(sql, [userId]);
};
