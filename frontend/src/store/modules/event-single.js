...
export const mutations = {
  setEvent (state, payload) {
    ...
    state.event.reactions = {
      like: Number.parseInt(reactions.like) || 0,
      heart: Number.parseInt(reactions.heart) || 0,
      laugh: Number.parseInt(reactions.laugh) || 0,
      unlike: Number.parseInt(reactions.unlike) || 0,
      sad: Number.parseInt(reactions.sad) || 0,
      angry: Number.parseInt(reactions.angry) || 0,
    }
  ...
  setReactions (state, payload) {
    ...
    if (payload.counts) {
      ...
      state.event.reactions = {
        like: Number.parseInt(payload.counts.like) || 0,
        heart: Number.parseInt(payload.counts.heart) || 0,
        laugh: Number.parseInt(payload.counts.laugh) || 0,
        unlike: Number.parseInt(payload.counts.unlike) || 0,
        sad: Number.parseInt(payload.counts.sad) || 0,
        angry: Number.parseInt(payload.counts.angry) || 0,
      }
    } else if (payload.reactions) {
      state.event.reactions = {
        like: Number.parseInt(payload.reactions.like) || 0,
        heart: Number.parseInt(payload.reactions.heart) || 0,
        laugh: Number.parseInt(payload.reactions.laugh) || 0,
        unlike: Number.parseInt(payload.reactions.unlike) || 0,
        sad: Number.parseInt(payload.reactions.sad) || 0,
        angry: Number.parseInt(payload.reactions.angry) || 0,
      }
    }
  ...
  updateReaction (state, { reactionType, isActive, previousReaction }) {
    ...
    if (isActive && reactionType) {
      ...
      if (previousReaction === reactionType) {
        ...
        if (previousReaction && previousReaction !== reactionType && state.event.reactions[previousReaction] > 0) {
...
export const actions = {
  ...
  async addComment ({ commit }, request) {
    try {
      ...
      // Mentions should already be included in the response from backend
  ...
  async switchFavoriteEvent ({ commit, state }, request) {
    try {
      const params = { eventId: request.eventId, payload: request.payload }
      if (request.collectionId !== undefined && request.collectionId !== null) {
        params.collectionId = request.collectionId
      }
      const response = await $axios.get('/event/switchFavoriteEvent', { params })
      commit('setIsFavorite', response.data.payload)
      request.payload
        ? commit(`eventCollection/addEvent`, state.event, { root: true })
        : commit(`eventCollection/removeEvent`, state.event?.id, { root: true })
      return response
    } catch (error) {
      throw error
    }
